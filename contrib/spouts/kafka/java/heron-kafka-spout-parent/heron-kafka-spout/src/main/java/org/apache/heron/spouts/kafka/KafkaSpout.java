...
public class KafkaSpout<K, V> extends BaseRichSpout
  ...
  public void ack(Object msgId) {
    ...
    /*
      ...
      This happens when a failed tuple has caused Kafka consumer to seek back to earlier position,
      and some messages are replayed.
     */
  ...
  private void emitConsumerRecord(ConsumerRecord<K, V> record) {
    ...
    tupleByStream.forEach((s, objects) -> {
      if (topologyReliabilityMode != Config.TopologyReliabilityMode.ATLEAST_ONCE) {
        ...
        //only in effective once mode, we need to track the offset of the record
        //that is just emitted into the topology
        ...
        ConsumerRecordMessageId consumerRecordMessageId =
            new ConsumerRecordMessageId(new TopicPartition(record.topic(), record.partition()),
                record.offset());
  ...
  private void rewindAndDiscardAck(TopicPartition topicPartition,
    ...
    if (failureRegistry.containsKey(topicPartition)) {
      ...
      //if there is any because we've rewound the consumer back
  ...
  private void manualCommit(TopicPartition topicPartition, NavigableMap<Long, Long> ackRanges) {
    //the first entry in the acknowledgement registry keeps track of the lowest possible offset
