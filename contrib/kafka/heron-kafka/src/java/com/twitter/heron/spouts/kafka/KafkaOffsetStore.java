...
public class KafkaOffsetStore extends MetadataStore {
  ...
  // as there is a manager per topic per partition and the stateUpdateIntervalMs should not be
  // too small
  ...
  private synchronized BlockingChannel locateOffsetManager() {
    if (storeOffsetManager == null) {
      ...
      int maxRetry = 3;
      int retryCount = 0;
      // this usually only happens when the internal offsets topic does not exist before and we
      // need to wait until
      ...
      while (retryCount < maxRetry) {
        channel.send(new kafka.api.GroupCoordinatorRequest(storeConsumerGroupId, kafka.api
        ...
        if (groupCoordinatorResponse.errorCode() == ErrorMapping
      ...
      if (groupCoordinatorResponse.errorCode() == ErrorMapping.NoError()) {
        ...
        throw new RuntimeException("Kafka metadata fetch error: " + groupCoordinatorResponse
            .errorCode());
      }
