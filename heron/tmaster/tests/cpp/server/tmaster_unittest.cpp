                  ...
                  sp_int32 tmaster_port, sp_int32 tmaster_controller_port)
{
  ...
  tmaster =
    new heron::tmaster::TMaster(zkhostportlist, topology_name, topology_id,
                                ...
                                tmaster_port, tmaster_port+2, tmaster_port+3, LOCALHOST, ss);
...
void StartDummyStMgr(EventLoopImpl*& ss, heron::testing::DummyStMgr*& mgr,
                     ...
                     const sp_string& stmgr_id, sp_int32 stmgr_port,
                     const std::vector<heron::proto::system::Instance*>& instances)
{
  ...
  mgr = new heron::testing::DummyStMgr(ss, options, stmgr_id,
                                       LOCALHOST, stmgr_port,
                                       instances);
  mgr->Start();
...
StartTMaster(CommonResources& common)
{
  ...
  StartTMaster(tmaster_eventLoop, common.tmaster_, common.tmaster_thread_,
               ...
               common.tmaster_controller_port_);
...
StartStMgrs(CommonResources& common)
{
  ...
  for (int i = 0; i < common.num_stmgrs_; ++i) {
    ...
    StartDummyStMgr(stmgr_ss, mgr, stmgr_thread,
               LOCALHOST, common.tmaster_port_,
...
void
SetUpCommonResources(CommonResources& common)
{
...
void ControlTopologyDone(HTTPClient* client, IncomingHTTPResponse* response)
{
  if (response->response_code() != 200) {
    FAIL() << "Error while controlling topology "
...
void ControlTopology(sp_string topology_id, sp_int32 port, bool activate)
{
  ...
  OutgoingHTTPRequest* request = new OutgoingHTTPRequest(LOCALHOST, port,
                                                         requesturl,
                                                         BaseHTTPRequest::GET, kvs);
  auto cb = [=] (IncomingHTTPResponse* response) {
    ControlTopologyDone(client, response);
  };
...
TEST(StMgr, test_activate_deactivate)
{
  ...
  std::thread* deactivate_thread = new std::thread(ControlTopology,
                                         common.topology_id_,
                                         common.tmaster_controller_port_,
                                         false);
  ...
  std::thread* activate_thread = new std::thread(ControlTopology,
                                       common.topology_id_,
                                       common.tmaster_controller_port_,
                                       true);
