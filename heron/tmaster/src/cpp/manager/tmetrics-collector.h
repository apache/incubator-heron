...
// TODO: Store metrics persistently to prevent against crashes.
class TMetricsCollector
{
 public:
  // _max_interval is how far along we keep individual metric blobs.
  TMetricsCollector(sp_int32 _max_interval, EventLoop* eventLoop, const std::string& metrics_sinks_yaml);
 ...
 private:
  ...
  struct TimeBucket {
    ...
    TimeBucket(sp_int32 bucket_interval) {
  ...
  // TODO: Use proto to store this data structure.
  class Metric {
   public:
    // TODO: Add ctor for default UNKNOWN type and give a set type function.
    Metric(const sp_string& name,
  ...
  class InstanceMetrics {
   public:
    ...
    InstanceMetrics(const sp_string& instance_id,
                    sp_int32 nbuckets, sp_int32 bucket_interval);
    // dtor
    ...
    void GetMetrics(const proto::tmaster::MetricRequest& request,
                    sp_int64 start_time, sp_int64 end_time,
                    proto::tmaster::MetricResponse* response);
  ...
  class ComponentMetrics {
   public:
    ...
    ComponentMetrics(const sp_string& component_name,
                     sp_int32 nbuckets, sp_int32 bucket_interval);
    // dtor
    ...
    void AddMetricForInstance(const sp_string& instance_id,
