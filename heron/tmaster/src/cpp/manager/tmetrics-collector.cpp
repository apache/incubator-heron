  ...
  start_time_(time(NULL))
{
  ...
  CHECK(max_interval_ % interval_ == 0);
  nintervals_ = max_interval_ / interval_;
  auto cb = [this] (EventLoop::Status status) { this->Purge(status); };
  CHECK(eventLoop_->registerTimer(std::move(cb),
                           false, interval_ * 1000000) > 0);
}
...
void TMetricsCollector::AddMetricsForComponent(
    const sp_string& component_name, const proto::tmaster::MetricDatum& metrics_data) {
  ...
  component_metrics->AddMetricForInstance(
      metrics_data.instance_id(), name, type, metrics_data.value());
}

void TMetricsCollector::AddExceptionsForComponent(
...
void TMetricsCollector::GetExceptionsHelper(
    const ExceptionLogRequest& request, ExceptionLogResponse* exceptions) {
...
ExceptionLogResponse* TMetricsCollector::GetExceptions(
...
ExceptionLogResponse* TMetricsCollector::GetExceptionsSummary (
    const ExceptionLogRequest& request) {
  ExceptionLogResponse* response = new ExceptionLogResponse();
...
void TMetricsCollector::AggregateExceptions(
    const ExceptionLogResponse& all_exceptions, ExceptionLogResponse* aggregate_exceptions) {
  ...
  for (std::map<std::string, TmasterExceptionLog*>::iterator summary_iter = exception_summary.begin();
...
TMetricsCollector::ComponentMetrics::ComponentMetrics(
  ...
  : component_name_(component_name),
    nbuckets_(nbuckets), bucket_interval_(bucket_interval)
{
}
...
void TMetricsCollector::ComponentMetrics::AddMetricForInstance(
    ...
    const sp_string& name,
    TMasterMetrics::MetricAggregationType type,
...
void TMetricsCollector::ComponentMetrics::GetMetrics(
    const MetricRequest& _request,
  ...
  if (_request.instance_id_size() == 0) {
    ...
    for (sp_int32 i = 0; i < _request.instance_id_size(); ++i) {
      ...
      if (metrics_.find(id) == metrics_.end()) {
        LOG(ERROR) << "GetMetrics request received for unknown instance_id "
                   << id;
...
void TMetricsCollector::ComponentMetrics::GetExceptionsForInstance(
    const sp_string& instance_id,
...
void TMetricsCollector::ComponentMetrics::GetAllExceptions(
    ExceptionLogResponse* response) {
...
void TMetricsCollector::InstanceMetrics::AddExceptions(
    const TmasterExceptionLog& exception) {
  // TODO: Aggregate exceptions across minutely buckets. Try to avoid duplication of hash-fuction
  // used to aggregate in heron-worker.
  TmasterExceptionLog* new_exception = new TmasterExceptionLog();
  new_exception->CopyFrom(exception);
  exceptions_.push_back(new_exception);
  sp_uint32 max_exception =
    config::HeronInternalsConfigReader::Instance()->GetHeronTmasterMetricsCollectorMaximumException();
...
void TMetricsCollector::InstanceMetrics::GetMetrics(
...
TMetricsCollector::Metric::Metric(const sp_string& name,
  ...
  : name_(name), metric_type_(type),
    all_time_cumulative_(0), all_time_nitems_(0),
    bucket_interval_(bucket_interval)
{
...
void TMetricsCollector::Metric::GetMetrics(bool minutely,
                                           sp_int64 start_time,
                                           sp_int64 end_time,
                                           IndividualMetric* _response)
{
  ...
  if (minutely) {
    ...
    for (std::list<TimeBucket*>::iterator iter = data_.begin();
         iter != data_.end(); ++iter) {
