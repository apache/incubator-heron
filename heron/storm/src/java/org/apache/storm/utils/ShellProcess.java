...
public class ShellProcess implements Serializable {
  ...
  private void modifyEnvironment(Map<String, String> buildEnv) {
    for (Map.Entry<String, String> entry : env.entrySet()) {
      if ("PATH".equals(entry.getKey())) {
        buildEnv.put("PATH",
            buildEnv.get("PATH") + File.pathSeparatorChar + env.get("PATH"));
      } else {
  ...
  private ISerializer getSerializer(Map conf) {
    ...
    String serializer_className =
        (String) conf.get(Config.TOPOLOGY_MULTILANG_SERIALIZER);
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to construct multilang serializer from serializer "
  ...
  /**
   *
   * @return exit code of the process if process is terminated, -1 if process is
  ...
  public String getProcessTerminationInfoString() {
    return String.format(" exitCode:%s, errorString:%s ", getExitCode(),
        getErrorsString());
  }
}
