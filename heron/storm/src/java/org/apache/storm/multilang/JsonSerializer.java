...
public class JsonSerializer implements ISerializer {
  ...
  public void initialize(OutputStream processIn, InputStream processOut) {
    try {
      this.processIn = new BufferedWriter(
          new OutputStreamWriter(processIn, DEFAULT_CHARSET));
      this.processOut = new BufferedReader(
          new InputStreamReader(processOut, DEFAULT_CHARSET));
    ...
    while (true) {
      ...
      if (subline == null) {
        ...
        if (line.length() == 0) {
          ...
          errorMessage
              .append(" Currently read output: " + line.toString() + "\n");
        }
