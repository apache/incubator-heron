 ...
 * in the resources directory within the jar submitted to the master. During
 * development/testing on a local machine, that resources directory just needs
 * to be on the classpath.
 *
 * When creating topologies using the Java API, subclass this bolt and implement
 * the IRichBolt interface to create components for the topology that use other
 * languages. For example:
 *
 *
 * ```java public class MyBolt extends ShellBolt implements IRichBolt { public
 * MyBolt() { super("python", "mybolt.py"); }
 *
...
public class ShellBolt implements IBolt {
  ...
  public void prepare(Map stormConf, TopologyContext context,
    ...
    if (maxPending != null) {
      this._pendingWrites =
          new ShellBoltMessageQueue(((Number) maxPending).intValue());
    }
    ...
    if (stormConf.containsKey(Config.TOPOLOGY_SUBPROCESS_TIMEOUT_SECS)) {
      workerTimeoutMills = 1000 * Utils
          .getInt(stormConf.get(Config.TOPOLOGY_SUBPROCESS_TIMEOUT_SECS));
    } else {
      workerTimeoutMills = 1000
          * Utils.getInt(stormConf.get(Config.SUPERVISOR_WORKER_TIMEOUT_SECS));
    }
    ...
    heartBeatExecutorService.scheduleAtFixedRate(
        new BoltHeartbeatTimerTask(this), 1, 1, TimeUnit.SECONDS);
  }
  ...
  public void execute(Tuple input) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Error during multilang processing " + processInfo, e);
    }
  }
  ...
  private void handleEmit(ShellMsg shellMsg) throws InterruptedException {
    ...
    if (recvAnchors != null) {
      for (String anchor : recvAnchors) {
        ...
        if (t == null) {
          throw new RuntimeException(
              "Anchored onto " + anchor + " after ack/fail");
        }
        anchors.add(t);
      }
    }
    ...
    if (shellMsg.getTask() == 0) {
      List<Integer> outtasks =
          _collector.emit(shellMsg.getStream(), anchors, shellMsg.getTuple());
      if (shellMsg.areTaskIdsNeeded()) {
        _pendingWrites.putTaskIds(outtasks);
      }
    } else {
      _collector.emitDirect((int) shellMsg.getTask(), shellMsg.getStream(),
        ...
        try {
          ...
          if (command == null) {
            throw new IllegalArgumentException(
                "Command not found in bolt message: " + shellMsg);
          }

          setHeartbeat();

          // We don't need to take care of sync, cause we're always updating
          // heartbeat
          switch (command) {
          case "ack":
