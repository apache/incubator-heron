...
@SuppressWarnings({"unchecked", "rawtypes", "checkstyle:all"})
    ...
    public StormTopology buildTopology() {
        ...
        for(String id: _bolts.keySet()) {
            ...
            for(GlobalStreamId s: getBoltSubscriptionStreams(id)) {
                ...
                if (batch == null) {
                  throw new RuntimeException(String.format(
                      "Batch group id not found for stream id %s in batchIdsForBolts: %s",
                      s, batchIdsForBolts));
                }
            ...
            for(String b: c.committerBatches) {
                String masterCoordinator = masterCoordinator(b);
                CoordSpec spec = specs.get(b);
                GlobalStreamId streamId = new GlobalStreamId(masterCoordinator, MasterBatchCoordinator.COMMIT_STREAM_ID);
                spec.commitStream = streamId;
            }
