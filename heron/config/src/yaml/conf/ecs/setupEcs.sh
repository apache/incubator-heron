...
##### PLEASE NOTE USE THIS AS A REFETECE. YOU MAY NEED TO CHANGE REGION/CLUSTER/KEYPAIR VALUES
...
# Key pair
echo -n "Creating Key Pair (ecs-heron-key, file ecs-heron-key.pem) .. "
aws ec2 create-key-pair --key-name ecs-heron-key --query 'KeyMaterial' --output text > ecs-heron-key.pem
chmod 600 ecs-heron-key.pem
echo "done"

# IAM role
echo -n "Creating IAM role (ecs-heron-role) .. "
aws iam create-role --role-name ecs-heron-role --assume-role-policy-document file://data/ecs-heron-role.json > /dev/null
aws iam put-role-policy --role-name ecs-heron-role --policy-name ecs-heron-policy --policy-document file://data/ecs-heron-policy.json
...
ecs-cli configure --region $REGION --access-key ecs-heron-key --cluster $CLUSTER
