...
StMgr::StMgr(EventLoop* eventLoop, sp_int32 _myport,
             const sp_string& _topology_name, const sp_string& _topology_id,
             proto::api::Topology* _hydrated_topology,
...
StMgr::Init() {
  ...
  state_mgr_ =
      heron::common::HeronStateMgr::MakeStateMgr(zkhostport_, zkroot_, eventLoop_, false);
  metrics_manager_client_ = new heron::common::MetricsMgrSt(
      IpUtils::getHostName(), stmgr_port_, metricsmgr_port_, "__stmgr__",
      stmgr_id_, metrics_export_interval_sec, eventLoop_);
  ...
  state_mgr_->SetTMasterLocationWatch(topology_name_,
    [this] () { this->FetchTMasterLocation(); });

  FetchTMasterLocation();

  CHECK_GT(eventLoop_->registerTimer(
            [this] (EventLoop::Status status) { this->CheckTMasterLocation(status); },
            false, config::HeronInternalsConfigReader::Instance()
                    ->GetCheckTMasterLocationIntervalSec() * 1000 * 1000), 0);  // fire only once
  ...
  CHECK_GT(eventLoop_->registerTimer(
            [] (EventLoop::Status) { ::heron::common::PruneLogs(); },
                           true, config::HeronInternalsConfigReader::Instance()
  ...
  CHECK_GT(eventLoop_->registerTimer(
            [] (EventLoop::Status) { ::heron::common::FlushLogs(); },
                           true, config::HeronInternalsConfigReader::Instance()
                                         ->GetHeronLoggingFlushIntervalSec() *
                                     1000 * 1000), 0);
...
bool StMgr::DidAnnounceBackPressure() {
  return server_->DidAnnounceBackPressure();
}
...
void StMgr::UpdateProcessMetrics(EventLoop::Status) {
  ...
  auto delta = duration_cast<seconds>(end_time - start_time_).count();
...
void StMgr::FetchTMasterLocation() {
  ...
  proto::tmaster::TMasterLocation* tmaster =
      new proto::tmaster::TMasterLocation();
...
void StMgr::StartStmgrServer() {
  ...
  server_ =
      new StMgrServer(eventLoop_, sops, topology_name_, topology_id_, stmgr_id_,
...
void StMgr::CreateTMasterClient(proto::tmaster::TMasterLocation* tmasterLocation) {
  ...
  auto pplan_watch = [this] (proto::system::PhysicalPlan* pplan) {
    this->NewPhysicalPlan(pplan);
  };

  tmaster_client_ = new TMasterClient(eventLoop_, master_options,
      stmgr_id_, stmgr_port_, shell_port_,
...
void StMgr::CreateTupleCache() {
  ...
  sp_uint32 drain_threshold_bytes_ =
      config::HeronInternalsConfigReader::Instance()
          ->GetHeronStreammgrCacheDrainSizeMb() * 1024 * 1024;
...
void StMgr::HandleNewTmaster(proto::tmaster::TMasterLocation* newTmasterLocation) {
  ...
  if (!clientmgr_) {
    clientmgr_ = new StMgrClientMgr(eventLoop_, topology_name_, topology_id_,
...
void StMgr::OnTMasterLocationFetch(
  proto::tmaster::TMasterLocation* newTmasterLocation,
  ...
  if (_status != proto::system::OK) {
    ...
    CHECK_GT(eventLoop_->registerTimer(
            [this] (EventLoop::Status) { this->FetchTMasterLocation(); }, false,
            TMASTER_RETRY_FREQUENCY), 0);
  } else {
    ...
    if (newTmasterLocation->topology_name() != topology_name_ ||
      ...
      LOG(FATAL) << "Topology name/id mismatch between stmgr and TMaster "
                 ...
                 << " but tmaster had " << newTmasterLocation->topology_name() << " : "
    ...
    if(tmaster_client_) {
      ...
      std::string newTmasterHostPort = newTmasterLocation->host() + ":" +
        std::to_string(newTmasterLocation->master_port());
      ...
      if(currentTmasterHostPort == newTmasterHostPort) {
        ...
        LOG(INFO) << "New tmaster location different from the current one."
                  << " Current one at " << currentTmasterHostPort
                  << " and New one at " << newTmasterHostPort << std::endl;
        isNewTmaster = true;
      }
    }
...
void StMgr::StartTMasterClient() {
  if(!tmaster_client_) {
    ...
    if (!tmaster_client_->IsConnected()) {
      LOG(INFO)
          << "Connecting to the TMaster as all the instances have connected to us"
          << std::endl;
...
void
StMgr::NewPhysicalPlan(proto::system::PhysicalPlan* _pplan) {
  ...
  if (!pplan_) {
    ...
    LOG(INFO) << "Topology state changed from "
              << _pplan->topology().state() << " to "
  ...
  for (sp_int32 i = 0; i < _pplan->instances_size(); ++i) {
    ...
    const sp_string& component_name =
        _pplan->instances(i).info().component_name();
    if (component_to_task_ids.find(component_name) ==
        component_to_task_ids.end()) {
  ...
  if (!pplan_) {
    ...
    PopulateXorManagers(_pplan->topology(),
                        ExtractTopologyTimeout(_pplan->topology()),
...
void StMgr::CleanupStreamConsumers() {
  ...
  for (iter = stream_consumers_.begin(); iter != stream_consumers_.end();
       ++iter) {
    delete iter->second;
  }
...
sp_int32 StMgr::ExtractTopologyTimeout(const proto::api::Topology& _topology) {
  for (sp_int32 i = 0; i < _topology.topology_config().kvs_size(); ++i) {
    if (_topology.topology_config().kvs(i).key() ==
...
void StMgr::PopulateStreamConsumers(
  ...
  for (sp_int32 i = 0; i < _topology->spouts_size(); ++i) {
    for (sp_int32 j = 0; j < _topology->spouts(i).outputs_size(); ++j) {
      proto::api::OutputStream* os =
          _topology->mutable_spouts(i)->mutable_outputs(j);
  ...
  for (sp_int32 i = 0; i < _topology->bolts_size(); ++i) {
    for (sp_int32 j = 0; j < _topology->bolts(i).outputs_size(); ++j) {
      proto::api::OutputStream* os =
          _topology->mutable_bolts(i)->mutable_outputs(j);
      std::pair<sp_string, sp_string> p =
          make_pair(os->stream().component_name(), os->stream().id());
      schema_map[p] = os->mutable_schema();
    }
  ...
  for (sp_int32 i = 0; i < _topology->bolts_size(); ++i) {
    for (sp_int32 j = 0; j < _topology->bolts(i).inputs_size(); ++j) {
      ...
      std::pair<sp_string, sp_string> p =
          make_pair(is.stream().component_name(), is.stream().id());
      proto::api::StreamSchema* schema = schema_map[p];
      const sp_string& component_name = _topology->bolts(i).comp().name();
      CHECK(_component_to_task_ids.find(component_name) !=
            _component_to_task_ids.end());
      ...
      if (stream_consumers_.find(p) == stream_consumers_.end()) {
        stream_consumers_[p] =
            new StreamConsumers(is, *schema, component_task_ids);
      } else {
...
void StMgr::ProcessAcksAndFails(
  ...
  for (sp_int32 i = 0; i < _control.emits_size(); ++i) {
    ...
    for (sp_int32 j = 0; j < ack_tuple.roots_size(); ++j) {
      ...
      CHECK(!xor_mgrs_->anchor(_task_id, ack_tuple.roots(j).key(),
                               ack_tuple.ackedtuple()));
    }
  }
  ...
  for (sp_int32 i = 0; i < _control.acks_size(); ++i) {
    ...
    for (sp_int32 j = 0; j < ack_tuple.roots_size(); ++j) {
      CHECK_EQ(_task_id, ack_tuple.roots(j).taskid());
      if (xor_mgrs_->anchor(_task_id, ack_tuple.roots(j).key(),
                            ack_tuple.ackedtuple())) {
        // This tuple tree is all over
        proto::system::AckTuple* a =
            out.mutable_set()->mutable_control()->add_acks();
    ...
    for (sp_int32 j = 0; j < fail_tuple.roots_size(); ++j) {
      ...
      if (xor_mgrs_->remove(_task_id, fail_tuple.roots(j).key())) {
        ...
        proto::system::AckTuple* f =
            out.mutable_set()->mutable_control()->add_fails();
...
void StMgr::HandleInstanceData(const sp_int32 _src_task_id, bool _local_spout,
  ...
  if (_message->set().has_data()) {
    proto::system::HeronDataTupleSet* d =
        _message->mutable_set()->mutable_data();
    ...
    if (stream_consumers_.find(stream) != stream_consumers_.end()) {
      ...
      for (sp_int32 i = 0; i < d->tuples_size(); ++i) {
        ...
        CopyDataOutBound(_src_task_id, _local_spout, d->stream(), d->tuples(i),
                         out_tasks);
      }
    } else {
      LOG(ERROR) << "Nobody consumes stream " << stream.second
                 << " from component " << stream.first;
    }
  }
  if (_message->set().has_control()) {
    proto::system::HeronControlTupleSet* c =
        _message->mutable_set()->mutable_control();
...
void StMgr::DrainInstanceData(sp_int32 _task_id,
  ...
  if (dest_stmgr_id == stmgr_id_) {
    ...
    proto::stmgr::TupleStreamMessage* out =
        new proto::stmgr::TupleStreamMessage();
...
void StMgr::CopyControlOutBound(const proto::system::AckTuple& _control,
                                bool _is_fail) {
...
void StMgr::CopyDataOutBound(sp_int32 _src_task_id, bool _local_spout,
  ...
  for (std::list<sp_int32>::const_iterator iter = _out_tasks.begin();
       iter != _out_tasks.end(); ++iter) {
    ...
    if (_tuple.roots_size() > 0) {
      ...
      if (_local_spout) {
        ...
        if (first_iteration) {
          ...
          CHECK(!xor_mgrs_->anchor(_src_task_id, _tuple.roots(0).key(),
                                   tuple_key));
        }
      } else {
