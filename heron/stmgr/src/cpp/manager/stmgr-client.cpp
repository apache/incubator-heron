...
const sp_string METRIC_ACK_TUPLES_TO_STMGRS_LOST =
    "__ack_tuples_to_stmgrs_lost";
// Num fail tuples lost while sending to other stream managers
const sp_string METRIC_FAIL_TUPLES_TO_STMGRS_LOST =
    "__fail_tuples_to_stmgrs_lost";
...
StMgrClient::StMgrClient(EventLoop* eventLoop, const NetworkOptions& _options,
  ...
  InstallResponseHandler(new proto::stmgr::StrMgrHelloRequest(),
...
void StMgrClient::HandleConnect(NetworkErrorCode _status) {
  if (_status == OK) {
    ...
    LOG(WARNING) << "Could not connect to stmgr " << other_stmgr_id_
                 << " running at " << get_clientoptions().get_host() << ":"
                 << get_clientoptions().get_port() << " due to: " << _status
                 << std::endl;
    if (quit_) {
      LOG(ERROR) << "Quitting";
      delete this;
...
void StMgrClient::HandleClose(NetworkErrorCode _code) {
  if (_code == OK) {
    LOG(INFO) << "We closed our server connection with stmgr "
              << other_stmgr_id_ << " running at "
              << get_clientoptions().get_host() << ":"
              << get_clientoptions().get_port() << std::endl;
  } else {
    LOG(INFO) << "Stmgr " << other_stmgr_id_ << " running at "
              << get_clientoptions().get_host() << ":"
...
void StMgrClient::HandleHelloResponse(
    void*, proto::stmgr::StrMgrHelloResponse* _response,
    NetworkErrorCode _status) {
  if (_status != OK) {
    LOG(ERROR) << "NonOK network code " << _status
               << " for register response from stmgr " << other_stmgr_id_
               << " running at " << get_clientoptions().get_host() << ":"
               << get_clientoptions().get_port();
    delete _response;
    Stop();
  ...
  if (status != proto::system::OK) {
    LOG(ERROR) << "NonOK register response " << status << " from stmgr "
               << other_stmgr_id_ << " running at "
               << get_clientoptions().get_host() << ":"
               << get_clientoptions().get_port();
    Stop();
...
void StMgrClient::SendHelloRequest() {
  proto::stmgr::StrMgrHelloRequest* request =
      new proto::stmgr::StrMgrHelloRequest();
...
void StMgrClient::SendTupleStreamMessage(
    proto::stmgr::TupleStreamMessage* _msg) {
  if (IsConnected()) {
    stmgr_client_metrics_->scope(METRIC_BYTES_TO_STMGRS)
        ->incr_by(_msg->ByteSize());
    ...
    stmgr_client_metrics_->scope(METRIC_BYTES_TO_STMGRS_LOST)
        ->incr_by(_msg->ByteSize());
    if (_msg->set().has_data()) {
    ...
    if (++ndropped_messages_ % 100 == 0) {
      LOG(INFO) << "Dropping " << ndropped_messages_
                << "th tuple message to stmgr " << other_stmgr_id_
...
void StMgrClient::SendStartBackPressureMessage() {
  ...
  proto::stmgr::StartBackPressureMessage* message =
      new proto::stmgr::StartBackPressureMessage();
...
void StMgrClient::SendStopBackPressureMessage() {
  ...
  REQID rand = generator.generate();
  // generator.generate(rand);
  proto::stmgr::StopBackPressureMessage* message =
      new proto::stmgr::StopBackPressureMessage();
  message->set_topology_name(topology_name_);
  message->set_topology_id(topology_id_);
  message->set_stmgr(our_stmgr_id_);
  message->set_message_id(rand.str());
