...
StMgrClientMgr::StMgrClientMgr(
    EventLoop* eventLoop, const sp_string& _topology_name,
  ...
  metrics_manager_client_->register_metric("__clientmgr",
                                           stmgr_clientmgr_metrics_);
}
...
void StMgrClientMgr::NewPhysicalPlan(
  ...
  for (sp_int32 i = 0; i < _pplan->stmgrs_size(); ++i) {
    ...
    if (clients_.find(s.id()) != clients_.end()) {
      ...
      if (o.get_host() != s.host_name() || o.get_port() != s.data_port()) {
        LOG(INFO) << "Stmgr " << s.id() << " changed from " << o.get_host()
                  << ":" << o.get_port() << " to " << s.host_name() << ":"
                  << s.data_port();
      ...
      LOG(INFO) << "Stmgr " << s.id() << " came on " << s.host_name() << ":"
                << s.data_port();
      clients_[s.id()] = CreateClient(s.id(), s.host_name(), s.data_port());
    }
  }
  ...
  for (std::set<sp_string>::iterator iter = to_remove.begin();
...
StMgrClient* StMgrClientMgr::CreateClient(const sp_string& _other_stmgr_id,
  ...
  StMgrClient* client =
      new StMgrClient(eventLoop_, options, topology_name_, topology_id_, stmgr_id_,
                      _other_stmgr_id, this, metrics_manager_client_);
  client->Start();
  return client;
}

void StMgrClientMgr::SendTupleStreamMessage(
    const sp_string& _stmgr_id, proto::stmgr::TupleStreamMessage* _msg) {
  std::map<sp_string, StMgrClient*>::iterator iter;
...
void StMgrClientMgr::StartBackPressureOnServer(
    const sp_string& _other_stmgr_id) {
  stream_manager_->StartBackPressureOnServer(_other_stmgr_id);
}

void StMgrClientMgr::StopBackPressureOnServer(
    const sp_string& _other_stmgr_id) {
