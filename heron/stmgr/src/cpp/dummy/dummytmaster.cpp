...
TMasterServer::TMasterServer(EventLoop* eventLoop,
                             ...
                             const std::vector<sp_string>& _stmgrs)
  : Server(eventLoop, _options), pplan_(NULL),
    stmgrs_(_stmgrs)
{
...
void
TMasterServer::OnTopologyFetch(proto::system::StatusCode _status)
{
  ...
  state_mgr_->SetTMasterLocation(tmaster_location, CreateCallback(this,
                                 &TMasterServer::OnTMasterSet));
}

void
TMasterServer::OnTMasterSet(proto::system::StatusCode _status)
{
  CHECK(_status == proto::system::OK);
...
TMasterServer::GeneratePplan()
{
  ...
  for (sp_int32 i = 0; i < topology_->spouts_size(); ++i) {
    ...
    sp_int32 parallelism = heron::config::TopologyConfigHelper::GetComponentParallelism(spout.comp().config());
    for (sp_int32 j = 0; j < parallelism; ++j) {
      proto::system::Instance* instance = pplan_->add_instances();
      std::ostringstream w; w << "instance-" << global_index;
  ...
  for (sp_int32 i = 0; i < topology_->bolts_size(); ++i) {
    ...
    sp_int32 parallelism = heron::config::TopologyConfigHelper::GetComponentParallelism(bolt.comp().config());
    for (sp_int32 j = 0; j < parallelism; ++j) {
      proto::system::Instance* instance = pplan_->add_instances();
      std::ostringstream w; w << "instance-" << global_index;
      instance->set_instance_id(w.str());
...
void TMasterServer::HandleConnectionClose(Connection*,
                                          NetworkErrorCode)
{
