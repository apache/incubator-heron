...
{
  if (argc != 9) {
    std::cout << "Usage: " << argv[0] << " <topname> <topid> <zknode> <zkroot> <stmgrid> <spoutworkerids> <boltworkerids> <port>" << std::endl;
  ...
  heron::stmgr::StMgr mgr(&ss, sops, topology_name, myid,
                          spout_workers, bolt_workers,
