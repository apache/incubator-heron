...
void OneTimer(heron::stmgr::XorManager* _g,
              std::vector<sp_int64>* _task_ids,
...
void TwoTimer(heron::stmgr::XorManager* _g,
              std::vector<sp_int64>* _task_ids,
              EventLoopImpl::Status)
{
...
TEST(XorManager, test_all)
{
  ...
  auto cb1 = [=] (EventLoopImpl::Status status ) { OneTimer(g, one_added, status); };
  ...
  auto cb2 = [=] (EventLoopImpl::Status status ) { TwoTimer(g, two_added, status); };
