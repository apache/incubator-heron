...
DummyInstance::DummyInstance(EventLoopImpl* eventLoop, const NetworkOptions& _options,
  ...
  : Client(eventLoop, _options), topology_name_(_topology_name), topology_id_(_topology_id),
    instance_id_(_instance_id), component_name_(_component_name),
    task_id_(_task_id), component_index_(_component_index),
    stmgr_id_(_stmgr_id), recvd_stmgr_pplan_(NULL),
...
void
DummyInstance::HandleConnect(NetworkErrorCode _status)
{
  if (_status == OK) {
...
void
DummyInstance::HandleClose(NetworkErrorCode)
{
}

heron::proto::system::StatusCode
DummyInstance::GetRegisterResponseStatus() {
    return register_response_status;
}

void
DummyInstance::HandleInstanceResponse(heron::proto::stmgr::RegisterInstanceResponse* _message)
{
...
void
DummyInstance::HandleTupleMessage(heron::proto::stmgr::TupleMessage*)
{
}

void
DummyInstance::HandleNewInstanceAssignmentMsg(heron::proto::stmgr::NewInstanceAssignmentMessage*)
{
}

void
DummyInstance::CreateAndSendInstanceRequest()
{
...
void
DummyInstance::CreateAndSendTupleMessages()
{
}
...
DummySpoutInstance::DummySpoutInstance(EventLoopImpl* eventLoop, const NetworkOptions& _options,
                                       ...
                                       sp_int32 _task_id, sp_int32 _component_index,
                                       const sp_string& _stmgr_id, const sp_string& stream_id,
                                       sp_int32 max_msgs_to_send,
                                       bool _do_custom_grouping) :
  DummyInstance(eventLoop, _options, _topology_name, _topology_id,
                _instance_id, _component_name, _task_id, _component_index, _stmgr_id),
  stream_id_(stream_id), max_msgs_to_send_(max_msgs_to_send),
  total_msgs_sent_(0), batch_size_(1000),
  do_custom_grouping_(_do_custom_grouping)
{
}

void
DummySpoutInstance::HandleInstanceResponse(heron::proto::stmgr::RegisterInstanceResponse* _message)
{
  DummyInstance::HandleInstanceResponse(_message);
}

void
DummySpoutInstance::HandleNewInstanceAssignmentMsg(heron::proto::stmgr::NewInstanceAssignmentMessage* _msg)
{
...
void
DummySpoutInstance::CreateAndSendTupleMessages()
{
  for (int i = 0 ; (i < batch_size_) && (total_msgs_sent_ < max_msgs_to_send_);
       ++total_msgs_sent_, ++i) {
    heron::proto::stmgr::TupleMessage* message =
      new heron::proto::stmgr::TupleMessage();
...
DummyBoltInstance::DummyBoltInstance(EventLoopImpl* eventLoop, const NetworkOptions& _options,
  ...
  DummyInstance(eventLoop, _options, _topology_name, _topology_id,
                _instance_id, _component_name, _task_id, _component_index, _stmgr_id),
  expected_msgs_to_recv_(_expected_msgs_to_recv), msgs_recvd_(0)
{
}

void
DummyBoltInstance::HandleInstanceResponse(heron::proto::stmgr::RegisterInstanceResponse* _message)
{
  DummyInstance::HandleInstanceResponse(_message);
}

void
DummyBoltInstance::HandleTupleMessage(heron::proto::stmgr::TupleMessage* msg)
{
  if (msg->set().has_data())
    msgs_recvd_ += msg->mutable_set()->mutable_data()->tuples_size();
  if (msgs_recvd_ >= expected_msgs_to_recv_)
    getEventLoop()->loopExit();
}

void
DummyBoltInstance::HandleNewInstanceAssignmentMsg(heron::proto::stmgr::NewInstanceAssignmentMessage* _msg)
{
  DummyInstance::HandleNewInstanceAssignmentMsg(_msg);
