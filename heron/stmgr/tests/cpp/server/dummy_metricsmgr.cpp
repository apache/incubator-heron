...
DummyMtrMgr::DummyMtrMgr(EventLoopImpl* ss, const NetworkOptions& options,
                         ...
                         CountDownLatch* tmasterLatch,
                         CountDownLatch* connectionCloseLatch)
  : Server(ss, options), stmgr_id_expected_(stmgr_id), location_(NULL),
    tmasterLatch_(tmasterLatch), connectionCloseLatch_(connectionCloseLatch)
{
...
DummyMtrMgr::~DummyMtrMgr()
{
  delete location_;
}

void
DummyMtrMgr::HandleNewConnection(Connection*) {
  LOG(INFO) << "Got a new connection";
}

void
DummyMtrMgr::HandleConnectionClose(Connection*,
...
void
DummyMtrMgr::HandleMetricPublisherRegisterRequest(REQID id, Connection* conn,
                                    heron::proto::system::MetricPublisherRegisterRequest* request)
{
...
void
DummyMtrMgr::HandleMetricPublisherPublishMessage(Connection*,
...
void
DummyMtrMgr::HandleTMasterLocationMessage(Connection*,
                                          heron::proto::system::TMasterLocationRefreshMessage* message)
{
  location_ = message->release_tmaster();
  delete message;

  LOG(INFO) << "Got tmaster location: " << location_->host() << ":"
            << location_->master_port();
