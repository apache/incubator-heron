 ...
 */
public class MetricsCacheQueryUtils {
  ...
  public static TopologyMaster.MetricResponse Convert(MetricCacheResponse response) {
    ...
    switch (response.status.status) {
      case 1:
        ret.setStatus(Common.Status.newBuilder().setStatus(Common.StatusCode.OK));
        break;
      case 2:
        ret.setStatus(Common.Status.newBuilder().setStatus(Common.StatusCode.NOTOK));
        break;
      default:
        ret.setStatus(Common.Status.newBuilder().setStatus(Common.StatusCode.NOTOK));
        break;
    ...
    for (TaskMetric tm : response.metric) {
      TopologyMaster.MetricResponse.TaskMetric tm_ = Convert(tm);
      ret.addMetric(tm_);
    }
  ...
  private static TopologyMaster.MetricResponse.TaskMetric Convert(TaskMetric tm) {
    ...
    for (IndividualMetric im : tm.metric) {
      TopologyMaster.MetricResponse.IndividualMetric im_ = Convert(im);
      ret.addMetric(im_);
    }
  ...
  private static TopologyMaster.MetricResponse.IndividualMetric Convert(IndividualMetric im) {
    ...
    for (IntervalValue iv : im.intervalValues) {
      TopologyMaster.MetricResponse.IndividualMetric.IntervalValue iv_ = Convert(iv);
      ret.addIntervalValues(iv_);
    }
