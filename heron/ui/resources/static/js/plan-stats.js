...
(function (global) {
  function drawStatsTable(planController, cluster, environ, toponame, physicalPlan, logicalPlan) {
    ...
    var rowData = [
      ...
      {
        name: 'Issues',
        get: function getAndRenderStats(statTableCell, justFillInTopLevel) {
          if (!justFillInTopLevel) {
            clearComponentColors(statTableCell);
          }
          querySniffer(function (result) {
            var seconds = statTableCell.time.seconds;
            var start = new Date().getTime() / 1000 - seconds;
            var durationString = statTableCell.time.name;
            var issues = result.filter(function (d) {
              return d.time > start;
            });
            ...
            issues.forEach(function (issue) {
              issue.instance_issues.forEach(function (instanceIssue) {
                byInstance[instanceIssue.instance] = byInstance[instanceIssue.instance] || {};
                instanceIssue.issues.forEach(function (description) {
                  byInstance[instanceIssue.instance][description] = true;
                });
              });
            });

            var numIssues = _.chain(issues)
              .pluck('instance_issues')
              .flatten()
              .pluck('issues')
              .uniq()
              .value().length;
            ...
            if (!justFillInTopLevel) {
              var instances = _.values(physicalPlan.result.instances);
              var byComponent = {};
              instances.forEach(function (instance) {
                byComponent[instance.name] = byComponent[instance.name] || 0;
                byComponent[instance.name] += _.size(byInstance[instance.id]);
                var tooltipDetails = '<br>' + _.keys(byInstance[instance.id]).join('<br>');
                colorInstances(_.size(byInstance[instance.id]), instance.id, statTableCell, tooltipDetails);
              });
              _.each(byComponent, function (count, name) {
                colorGraphNodes(count, name, statTableCell);
              });
            }

            colorTopLevel(numIssues, statTableCell);
          });
        },
        queryTrendline: function (metric, name, instance, start, end, callback) {
          var minuteToIndex = {};
          ...
          for (var i = start, j = 0; i < end; i+=60, j++) {
            minuteToIndex[i] = j;
            result[j] = [new Date(1000 * i), []];
          }

          function round(n) {
            return Math.floor(n / 60) * 60;
          }
          querySniffer(function (data) {
            issues = data.filter(function (d) {
              return start <= d.time && d.time <= end;
            });
            data.forEach(function (d) {
              time = round(d.time);
              var datapoint = result[minuteToIndex[time]];
              if (!datapoint) { return; }
              d.instance_issues.forEach(function (instanceIssue) {
                var id = instanceIssue.instance;
                var thisName = physicalPlan.result.instances[id].name;
                if ((id === instance || instance === '*') && (name === thisName || name === '*')) {
                  instanceIssue.issues.forEach(function (i) {
                    datapoint[1].push(i + id);
                  });
                }
              });
            });
            result.forEach(function (d) {
              d[1] = _.uniq(d[1]).length;
            });
            callback(result);
          });
        },
        tooltip: 'Issues found by Sniffer. More information at go/sniffer',
        loMedHi: [0, 0.5, 1],
        legendDescription: 'issues',
        scaleTicks: [0, 1],
        format: function (d) { return d.toFixed(0); },
      },
    ];
    ...
    window.querySniffer = querySniffer;
    function querySniffer(callback) {
      var snifferUrl = [
        '/topologies',
        cluster,
        environ,
      ...
      d3.json(snifferUrl, function (data) {
        // unpack the sniffer response into a list of time/issue pairs
        var result = {};
        _.each(data, function (data, time) {
          ...
          result[time].bad_containers = result[time].bad_containers.concat(_.keys(data.top_bad_containers));
          result[time].bad_components = result[time].bad_components.concat(_.keys(data.top_bad_components));
          result[time].bad_instances = result[time].bad_instances
            .concat(_.flatten(_.values(data.top_bad_containers)))
            .concat(_.flatten(_.values(data.top_bad_components)));
          _.each(data.top_bad_instances, function (instanceData, instance) {
            _.each(instanceData.history, function (issues, issueTime) {
              issueTime = Math.round(+issueTime / 60) * 60;
              result[issueTime] = result[issueTime] || {
                time: time,
                bad_containers: [],
                bad_components: [],
                bad_instances: [],
              ...
              result[issueTime].bad_instances.push(instance);
              result[issueTime].instance_issues[instance] = result[issueTime].instance_issues[instance] || {
                instance: instance,
                issues: []
              };
              result[issueTime].instance_issues[instance].issues = result[issueTime].instance_issues[instance].issues.concat(issues);
              result[issueTime].instance_issues[instance].issues = _.uniq(result[issueTime].instance_issues[instance].issues);
            });
          });
        });
        result = _.values(result);
        result.forEach(function (blob) {
          blob.bad_instances = _.uniq(blob.bad_instances);
          blob.instance_issues = _.values(blob.instance_issues);
        });
        callback(result);
      });
    }
