...
void
Server::SendResponse(REQID _id, Connection* _connection, const google::protobuf::Message& _response)
{
...
void
Server::CloseConnection(Connection* _connection)
{
  CloseConnection_Base(_connection);
}

void Server::AddTimer(VCallback<> cb, sp_int64 _msecs)
{
  AddTimer_Base(std::move(cb), _msecs);
}

void Server::SendRequest(Connection* _conn,
...
void Server::SendRequest(Connection* _conn,
                         ...
                         google::protobuf::Message* _response_placeholder)
{
  InternalSendRequest(_conn, _request, _msecs,
                      _response_placeholder, _ctx);
}

// The interfaces of BaseServer being implemented
BaseConnection*
Server::CreateConnection(ConnectionEndPoint* _endpoint,
                         ...
                         EventLoop*       eventLoop)
{
  ...
  auto npcb = [conn, this] (IncomingPacket* packet) {
    this->OnNewPacket(conn, packet);
  };
...
void
Server::HandleNewConnection_Base(BaseConnection* _connection)
{
  HandleNewConnection(static_cast<Connection *>(_connection));
}

void
Server::HandleConnectionClose_Base(BaseConnection*  _connection,
...
void
Server::OnNewPacket(Connection*     _connection,
                    IncomingPacket* _packet)
{
  ...
  if (active_connections_.find(_connection) == active_connections_.end()) {
    LOG(ERROR) << "Packet Received on on unknown connection "
               << _connection << " from hostport "
               << _connection->getIPAddress() << ":"
               << _connection->getPort();
    delete _packet;
  ...
  if (_packet->UnPackString(&typname) != 0) {
    LOG(ERROR) << "UnPackString failed from connection "
               << _connection << " from hostport "
               << _connection->getIPAddress() << ":"
               << _connection->getPort();
    delete _packet;
  ...
  if (requestHandlers.count(typname) > 0) {
    ...
    REQID rid;
    CHECK(_packet->UnPackREQID(&rid) == 0);
...
void
Server::InternalSendResponse(Connection*     _connection,
...
void
Server::InternalSendRequest(Connection* _conn,
                            google::protobuf::Message* _request,
                            sp_int64 _msecs,
                            google::protobuf::Message* _response_placeholder,
                            void* _ctx)
{
  ...
  // TODO:- If connection closes and there is no timeout associated with
  ...
  if (_msecs > 0) {
    auto cb = [rid, this] (EventLoop::Status status) {
      this->OnPacketTimer(rid, status);
    };
...
void Server::HandleResponse(google::protobuf::Message* _response,
                            void*, NetworkErrorCode)
{
  delete _response;
}

void Server::StartBackPressureConnectionCb(Connection*)
{
  // Nothing to be done here. Should be handled by inheritors if they care about backpressure
}

void Server::StopBackPressureConnectionCb(Connection*)
{
  // Nothing to be done here. Should be handled by inheritors if they care about backpressure
}
