...
Module_Errors::Module_Errors(const std::string&  _modname,
                             error_info_t*       _errs,
                             ...
                             sp_int32            _errcnt)
{
  ...
  for (sp_int32 i = 0; i < _errcnt; i++) {
    mod_errors_.push_back(
      Error_Info(
        _errs[i].errnum_, 
        std::string(_errs[i].errstr_), 
...
bool
Module_Errors::is_in_range(sp_uint32 _errno)
{
...
const std::string&
Module_Errors::get_error_msg(sp_uint32 _errno)
{
  return mod_errors_[_errno-errno_base_].get_errstr();
}

const std::string&
Module_Errors::get_errno_msg(sp_uint32 _errno)
{
  return mod_errors_[_errno-errno_base_].get_errno_str();
}

bool
Error_Store::load_module_errors(const std::string&                 _modname,
                                error_info_t*                      _errs,
...
bool
Error_Store::unload_module_errors(const std::string& _modname)
{
...
bool
Error_Store::unload_module_errors_all(void)
{
...
std::string
Error_Store::get_error_msg(sp_uint32 _errno)
{
  std::string                   msg;
...
std::string
Error_Store::get_module_name(sp_uint32 _errno)
{
...
std::string
Error_Store::get_errno_str(sp_uint32 _errno)
{
  std::string  msg;
...
std::string
Error_Store::get_syserr_str(sp_uint32 _errno)
{
  char           err_msg[BUFSIZ];
