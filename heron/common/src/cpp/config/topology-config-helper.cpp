                                                   ...
                                                   sp_int32 _parallelism)
{
  ...
  std::ostringstream ostr; ostr << _parallelism;
...
proto::api::Topology*
TopologyConfigHelper::StripComponentObjects(const proto::api::Topology& _topology)
{
...
sp_double64
TopologyConfigHelper::GetContainerCpuRequested(const proto::api::Topology& _topology)
{
  const proto::api::Config& cfg = _topology.topology_config();
  for (sp_int32 i = 0; i < cfg.kvs_size(); ++i) {
...
sp_int64
TopologyConfigHelper::GetContainerRamRequested(const proto::api::Topology& _topology)
{
  const proto::api::Config& cfg = _topology.topology_config();
  for (sp_int32 i = 0; i < cfg.kvs_size(); ++i) {
  ...
  sp_int32 total_parallelism = TopologyConfigHelper::GetTotalParallelism(_topology);
  sp_int32 nstmgrs = TopologyConfigHelper::GetNumStMgrs(_topology);
  sp_int64 max_components_per_container = (total_parallelism / nstmgrs) + \
                                          (total_parallelism % nstmgrs);
