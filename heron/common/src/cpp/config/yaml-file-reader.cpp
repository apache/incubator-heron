...
YamlFileReader::YamlFileReader(EventLoop* eventLoop,
...
void YamlFileReader::LoadConfig()
{
  CheckForChange(EventLoop::TIMEOUT_EVENT);
}
...
void YamlFileReader::CheckForChange(EventLoop::Status)
{
  ...
  if (last_changed > last_reload_) {
    LOG(INFO) << "Config file " << config_file_
              << " changed. reloading..";
    config_ = YAML::LoadFile(config_file_);
...
void YamlFileReader::AddIfMissing(const sp_string& _var,
                                  const sp_string& _default)
{
  if (!config_[_var]) {
    LOG(INFO) << "Adding missing config " << _var
              << " and setting its default value to " << _default << std::endl;
