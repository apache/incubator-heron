...
sp_int32
SockUtils::setNonBlocking(sp_int32 fd)
{
  sp_int32 flags;
...
sp_int32
SockUtils::getSendBufferSize(sp_int32 fd, sp_int32& size)
{
...
sp_int32
SockUtils::setSendBufferSize(sp_int32 fd, sp_int32 size)
{
  return ::setsockopt(fd, SOL_SOCKET, SO_SNDBUF, (char *)&size, sizeof(size));
}

sp_int32
SockUtils::getRecvBufferSize(sp_int32 fd, sp_int32& size)
{
  socklen_t optlen = sizeof(sp_int32);
...
sp_int32
SockUtils::setRecvBufferSize(sp_int32 fd, sp_int32 size)
{
  return ::setsockopt(fd, SOL_SOCKET, SO_RCVBUF, (char *)&size, sizeof(size));
}

sp_int32
SockUtils::setKeepAlive(sp_int32 fd)
{
  sp_int32 alive = 1;
  return ::setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, (char *)&alive, sizeof(alive));
}

sp_int32
SockUtils::setReuseAddress(sp_int32 fd)
{
  sp_int32 sopt = 1 ;
  return ::setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (char *)&sopt, sizeof(sopt));
}

sp_int32
SockUtils::setKeepIdleTime(sp_int32 fd, sp_int32 time)
{
#if defined(IS_MACOSX)
  sp_int32 on = 1 ;
  if (setsockopt(fd, SOL_SOCKET, SO_KEEPALIVE, &on, sizeof(on)) < 0)
    return SP_NOTOK;
  return ::setsockopt(fd, IPPROTO_TCP, TCP_KEEPALIVE, (char *)&time, sizeof(time));
#else
  return ::setsockopt(fd, SOL_TCP, TCP_KEEPIDLE, (char *)&time, sizeof(time));
#endif
}

sp_int32
SockUtils::setKeepIdleCount(sp_int32 fd, sp_int32 count)
{
#if defined(IS_MACOSX)
  return ::setsockopt(fd, IPPROTO_TCP, TCP_KEEPCNT, (char *)&count, sizeof(count));
#else
  return ::setsockopt(fd, SOL_TCP, TCP_KEEPCNT, (char *)&count, sizeof(count));
#endif
}

sp_int32
SockUtils::setKeepIdleInterval(sp_int32 fd, sp_int32 interval)
{
#if defined(IS_MACOSX)
  return ::setsockopt(fd, IPPROTO_TCP, TCP_KEEPINTVL, (char *)&interval, sizeof(interval));
#else
  return ::setsockopt(fd, SOL_TCP, TCP_KEEPINTVL, (char *)&interval, sizeof(interval));
#endif
}

sp_int32
SockUtils::setKeepIdleParams(
    sp_int32            fd,
    sp_int32            time,
    sp_int32            count,
    sp_int32            interval)
...
sp_int32
SockUtils::setTcpNoDelay(sp_int32 fd)
{
  sp_int32 on = 1;
...
sp_int32
SockUtils::setSocketDefaults(sp_int32 fd)
{
