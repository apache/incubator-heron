...
sp_int32
FileUtils::makeDirectory(const std::string& directory)
{
  struct stat st;
...
sp_int32
FileUtils::removeFile(const std::string& filepath)
{
...
sp_int32
FileUtils::removeRecursive(const std::string& directory, bool delete_self)
{
  ...
  while ((entry = ::readdir(dir)) != nullptr) {
    ...
    if (current_dir != 0 && parent_dir != 0) {
      ...
      if (entry->d_type == DT_DIR) {
        if (removeRecursive(path, true) == SP_NOTOK)
          return SP_NOTOK;
      } else {
...
std::string
FileUtils::readAll(const std::string& file)
{
...
time_t
FileUtils::getModifiedTime(const std::string& file)
{
  struct stat attrib;
...
bool
FileUtils::is_symlink(const std::string& filepath)
{
  struct stat attrib;
  ...
  size_t               len)
{
  ...
  if (!ot) return false;
  ot.write((char *)data, len);
  ot.close();
  return true;
}

sp_int32
FileUtils::getCwd(std::string& path)
{
