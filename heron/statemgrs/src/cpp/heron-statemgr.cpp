...
HeronStateMgr*
HeronStateMgr::MakeStateMgr(const std::string& _zk_hostport,
                            ...
                            bool exitOnSessionExpiry)
{
  if (_zk_hostport.empty()) {
    ...
    return new HeronZKStateMgr(_zk_hostport, _topleveldir, eventLoop,
                               exitOnSessionExpiry);
  }
}
...
void HeronStateMgr::ListExecutionStateDone(
    std::vector<proto::system::ExecutionState*>* _return,
    size_t _required_size,
    proto::system::ExecutionState* _s,
    VCallback<proto::system::StatusCode> cb,
...
std::string HeronStateMgr::GetTMasterLocationDir()
{
  return topleveldir_ + "/tmasters";
}

std::string HeronStateMgr::GetTopologyDir()
{
  return topleveldir_ + "/topologies";
}

std::string HeronStateMgr::GetPhysicalPlanDir()
{
  return topleveldir_ + "/pplans";
}

std::string HeronStateMgr::GetExecutionStateDir()
{
  return topleveldir_ + "/executionstate";
}
