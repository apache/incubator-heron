...
public class WaitQueueDisparityDetectorTest {
  ...
  public void testConfigAndFilter() {
    ...
    when(config.getConfig(CONF_DISPARITY_RATIO, "20")).thenReturn("15");

    ComponentMetrics compMetrics = new ComponentMetrics("bolt");
    compMetrics.addInstanceMetric(new InstanceMetrics("i1", METRIC_BUFFER_SIZE, 1501));
    compMetrics.addInstanceMetric(new InstanceMetrics("i2", METRIC_BUFFER_SIZE, 100));
    ...
    compMetrics.addInstanceMetric(new InstanceMetrics("i1", METRIC_BUFFER_SIZE, 1500));
    compMetrics.addInstanceMetric(new InstanceMetrics("i2", METRIC_BUFFER_SIZE, 110));
    topologyMetrics.put("bolt", compMetrics);
