...
HeronZKStateMgr::HeronZKStateMgr(const std::string& zkhostport,
                                 const std::string& topleveldir,
                                 EventLoop* eventLoop, bool exitOnSessionExpiry)
 : HeronStateMgr(topleveldir), zkhostport_(zkhostport), zkclient_(NULL),
   zkclient_factory_(new DefaultZKClientFactory()), eventLoop_(eventLoop),
   tmaster_location_watcher_info_(NULL), exitOnSessionExpiry_(exitOnSessionExpiry) {
  Init();
}
...
HeronZKStateMgr::HeronZKStateMgr(const std::string& zkhostport,
                  const std::string& topleveldir,
                  EventLoop* eventLoop,
                  ZKClientFactory* zkclient_factory, bool exitOnSessionExpiry)
 : HeronStateMgr(topleveldir), zkhostport_(zkhostport), zkclient_(NULL),
   zkclient_factory_(zkclient_factory), eventLoop_(eventLoop),
   tmaster_location_watcher_info_(NULL), exitOnSessionExpiry_(exitOnSessionExpiry)  {
  Init();
}
...
void
HeronZKStateMgr::SetTMasterLocationWatch(const std::string& topology_name,
                                         VCallback<> watcher)
{
  CHECK(watcher);
  ...
  tmaster_location_watcher_info_ =
      new TMasterLocationWatchInfo(std::move(watcher), topology_name);
  SetTMasterLocationWatchInternal();
}

void
HeronZKStateMgr::SetTMasterLocation(const proto::tmaster::TMasterLocation& _location,
...
void
HeronZKStateMgr::CreateTopology(const proto::api::Topology& _topology,
                                VCallback<proto::system::StatusCode> cb)
{
...
void
HeronZKStateMgr::DeleteTopology(const std::string& _topology_name,
                                VCallback<proto::system::StatusCode> cb)
{
...
void
HeronZKStateMgr::SetTopology(const proto::api::Topology& _topology,
                             VCallback<proto::system::StatusCode> cb)
{
  std::string path = GetTopologyPath(_topology.name());
  std::string value;
...
void HeronZKStateMgr::GetTopology(const std::string& _topology_name,
...
void
HeronZKStateMgr::CreatePhysicalPlan(const proto::system::PhysicalPlan& _pplan,
                                    VCallback<proto::system::StatusCode> cb)
{
  ...
  auto wCb = [cb, this] (sp_int32 rc) {
    this->CreatePhysicalPlanDone(std::move(cb), rc);
  };
  zkclient_->CreateNode(path, contents, false, std::move(wCb));
}

void
HeronZKStateMgr::DeletePhysicalPlan(const std::string& _topology_name,
                                    VCallback<proto::system::StatusCode> cb)
{
  std::string path = GetPhysicalPlanPath(_topology_name);
  auto wCb = [cb, this] (sp_int32 rc) {
    this->DeletePhysicalPlanDone(std::move(cb), rc);
  };
  zkclient_->DeleteNode(path, std::move(wCb));
}

void
HeronZKStateMgr::SetPhysicalPlan(const proto::system::PhysicalPlan& _pplan,
                                 VCallback<proto::system::StatusCode> cb)
{
  std::string path = GetPhysicalPlanPath(_pplan.topology().name());
  std::string contents;
  ...
  auto wCb = [cb, this] (sp_int32 rc) {
    this->SetPhysicalPlanDone(std::move(cb), rc);
  };
...
void
HeronZKStateMgr::CreateExecutionState(const proto::system::ExecutionState& _state,
                                      VCallback<proto::system::StatusCode> cb)
{
  ...
  auto wCb = [cb, this] (sp_int32 rc) {
    this->CreateExecutionStateDone(std::move(cb), rc);
  };
...
void
HeronZKStateMgr::DeleteExecutionState(const std::string& _topology_name,
                                      VCallback<proto::system::StatusCode> cb)
{
  std::string path = GetExecutionStatePath(_topology_name);
  auto wCb = [cb, this] (sp_int32 rc) {
    this->DeleteExecutionStateDone(std::move(cb), rc);
  };
...
void
HeronZKStateMgr::SetExecutionState(const proto::system::ExecutionState& _state,
                                   VCallback<proto::system::StatusCode> cb)
{
  std::string path = GetExecutionStatePath(_state.topology_name());
  std::string contents;
  _state.SerializeToString(&contents);
  auto wCb = [cb, this] (sp_int32 rc) {
    this->SetExecutionStateDone(std::move(cb), rc);
  };
...
void
HeronZKStateMgr::ListTopologies(std::vector<sp_string>* _return,
                                VCallback<proto::system::StatusCode> cb)
{
  sp_string path = GetTopologyDir();
  auto wCb = [cb, this] (sp_int32 rc) {
    this->ListTopologiesDone(std::move(cb), rc);
  };
...
void
HeronZKStateMgr::ListExecutionStateTopologies(std::vector<sp_string>* _return,
                                    VCallback<proto::system::StatusCode> cb)
{
  sp_string path = GetExecutionStateDir();
  auto wCb = [cb, this] (sp_int32 rc) {
    this->ListExecutionStateTopologiesDone(std::move(cb), rc);
  };
...
void HeronZKStateMgr::GlobalWatchEventHandler(const ZKClient::ZkWatchEvent event) {
  ...
  if(event.type == ZOO_SESSION_EVENT &&
    event.state == ZOO_EXPIRED_SESSION_STATE) {
...
void
HeronZKStateMgr::SetTMasterLocationDone(VCallback<proto::system::StatusCode> cb,
                                        sp_int32 _rc)
{
...
void
HeronZKStateMgr::GetTMasterLocationDone(std::string* _contents,
                                        proto::tmaster::TMasterLocation* _return,
                                        VCallback<proto::system::StatusCode> cb,
                                        sp_int32 _rc)
{
...
void
HeronZKStateMgr::CreateTopologyDone(VCallback<proto::system::StatusCode> cb,
                                    sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc == ZNONODE) {
...
void
HeronZKStateMgr::DeleteTopologyDone(VCallback<proto::system::StatusCode> cb,
                                    sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc == ZNONODE) {
...
void
HeronZKStateMgr::SetTopologyDone(VCallback<proto::system::StatusCode> cb,
                                 sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc == ZNONODE) {
...
void
HeronZKStateMgr::GetTopologyDone(std::string* _contents,
                                 proto::api::Topology* _return,
                                 VCallback<proto::system::StatusCode> cb,
                                 sp_int32 _rc)
{
...
void
HeronZKStateMgr::CreatePhysicalPlanDone(VCallback<proto::system::StatusCode> cb,
                                        sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc == ZNONODE) {
...
void
HeronZKStateMgr::DeletePhysicalPlanDone(VCallback<proto::system::StatusCode> cb,
                                        sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc == ZNONODE) {
...
void
HeronZKStateMgr::SetPhysicalPlanDone(VCallback<proto::system::StatusCode> cb,
                                     sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc == ZNONODE) {
...
void
HeronZKStateMgr::GetPhysicalPlanDone(std::string* _contents,
                                     proto::system::PhysicalPlan* _return,
                                     VCallback<proto::system::StatusCode> cb,
                                     sp_int32 _rc)
{
...
void
HeronZKStateMgr::CreateExecutionStateDone(VCallback<proto::system::StatusCode> cb,
                                          sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc == ZNONODE) {
    LOG(ERROR) << "Creating ExecutionState failed because zookeeper was not setup properly" << std::endl;
    code = proto::system::PATH_DOES_NOT_EXIST;
  } else if (_rc != ZOK) {
...
void
HeronZKStateMgr::DeleteExecutionStateDone(VCallback<proto::system::StatusCode> cb,
                                          sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc == ZNONODE) {
...
void
HeronZKStateMgr::SetExecutionStateDone(VCallback<proto::system::StatusCode> cb,
                                       sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc == ZNONODE) {
...
void
HeronZKStateMgr::GetExecutionStateDone(std::string* _contents,
                                       proto::system::ExecutionState* _return,
                                       VCallback<proto::system::StatusCode> cb,
                                       sp_int32 _rc)
{
...
void
HeronZKStateMgr::ListTopologiesDone(VCallback<proto::system::StatusCode> cb,
                                    sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc != ZOK) {
...
void
HeronZKStateMgr::ListExecutionStateTopologiesDone(VCallback<proto::system::StatusCode> cb,
                                    sp_int32 _rc)
{
  proto::system::StatusCode code = proto::system::OK;
  if (_rc != ZOK) {
...
bool HeronZKStateMgr::IsTmasterWatchDefined() {
  return (tmaster_location_watcher_info_ != NULL &&
...
void HeronZKStateMgr::SetTMasterWatchCompletionHandler(sp_int32 rc) {
  if (rc == ZOK || rc == ZNONODE) {
    ...
    LOG(INFO) << "Setting watch on tmaster location succeeded: " <<
        zerror(rc) << std::endl;
  } else {
    // Any other return code should be treated as warning, since ideally
    // we shouldn't be in this state.
    LOG(WARNING) << "Setting watch on tmaster location returned: " <<
        zerror(rc) << std::endl;

    if (ShouldRetrySetWatch(rc)) {
      LOG(INFO) << "Retrying after " << SET_WATCH_RETRY_INTERVAL_S <<
          " seconds" << std::endl;

      auto cb = [this] (EventLoop::Status status) {
        this->CallSetTMasterLocationWatch(status);
      };
...
void HeronZKStateMgr::SetTMasterLocationWatchInternal() {
  ...
  std::string path =
      GetTMasterLocationPath(tmaster_location_watcher_info_->topology_name);

  zkclient_->Exists(path,
    [this] () { this->TMasterLocationWatch(); },
    [this] (sp_int32 rc) { this->SetTMasterWatchCompletionHandler(rc); }
  );
}

void
HeronZKStateMgr::TMasterLocationWatch()
{
