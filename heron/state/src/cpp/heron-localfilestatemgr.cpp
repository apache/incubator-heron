...
void
HeronLocalFileStateMgr::InitTree()
{
...
void
HeronLocalFileStateMgr::SetTMasterLocationWatch(const std::string& topology_name,
                                                VCallback<> watcher)
{
  CHECK(watcher);
...
void
HeronLocalFileStateMgr::SetTMasterLocation(const proto::tmaster::TMasterLocation& _location,
                                           VCallback<proto::system::StatusCode> cb)
{
...
void
HeronLocalFileStateMgr::CreateTopology(const proto::api::Topology& _topology,
                                       VCallback<proto::system::StatusCode> cb)
{
...
void
HeronLocalFileStateMgr::DeleteTopology(const sp_string& _topology_name,
                                       VCallback<proto::system::StatusCode> cb)
{
...
void
HeronLocalFileStateMgr::SetTopology(const proto::api::Topology& _topology,
                                    VCallback<proto::system::StatusCode> cb)
{
  std::string fname = GetTopologyPath(_topology.name());
  std::string contents;
                                         ...
                                         VCallback<proto::system::StatusCode> cb)
{
  std::string contents;
  proto::system::StatusCode status =
          ReadAllFileContents(GetTopologyPath(_topology_name), contents);
  if (status == proto::system::OK) {
    if (!_return->ParseFromString(contents)) {
      status = proto::system::STATE_CORRUPTED;
    }
...
void
HeronLocalFileStateMgr::CreatePhysicalPlan(const proto::system::PhysicalPlan& _pplan,
                                           VCallback<proto::system::StatusCode> cb)
{
...
void
HeronLocalFileStateMgr::DeletePhysicalPlan(const sp_string& _topology_name,
                                           VCallback<proto::system::StatusCode> cb)
{
...
void
HeronLocalFileStateMgr::SetPhysicalPlan(const proto::system::PhysicalPlan& _pplan,
                                      VCallback<proto::system::StatusCode> cb)
{
  std::string contents;
  _pplan.SerializeToString(&contents);
...
void
HeronLocalFileStateMgr::CreateExecutionState(const proto::system::ExecutionState& _st,
                                             VCallback<proto::system::StatusCode> cb)
{
...
void
HeronLocalFileStateMgr::DeleteExecutionState(const std::string& _topology_name,
                                             VCallback<proto::system::StatusCode> cb)
{
...
void
HeronLocalFileStateMgr::SetExecutionState(
  const proto::system::ExecutionState&  _st,
  VCallback<proto::system::StatusCode> cb)
{
  std::string fname = GetExecutionStatePath(_st.topology_name());
  std::string contents;
...
void HeronLocalFileStateMgr::ListExecutionStateTopologies(
  std::vector<sp_string>*               _return,
  VCallback<proto::system::StatusCode> cb)
{
...
void HeronLocalFileStateMgr::ListTopologies(
  std::vector<sp_string>*               _return,
  VCallback<proto::system::StatusCode> cb)
{
  proto::system::StatusCode status = proto::system::OK;
...
HeronLocalFileStateMgr::ReadAllFileContents(const std::string& _filename, std::string& _contents)
{
...
proto::system::StatusCode
HeronLocalFileStateMgr::WriteToFile(const std::string& _filename, const std::string& _contents)
{
...
proto::system::StatusCode
HeronLocalFileStateMgr::DeleteFile(const std::string& _filename)
{
...
proto::system::StatusCode
HeronLocalFileStateMgr::MakeSureFileDoesNotExist(const std::string& _filename)
{
  std::ifstream in(_filename.c_str(), std::ios::in | std::ios::binary);
  if (in) {
    // it already exists.
...
void
HeronLocalFileStateMgr::CheckTMasterLocation(std::string topology_name,
                                             time_t last_change,
                                             VCallback<> watcher,
                                             EventLoop::Status)
{
