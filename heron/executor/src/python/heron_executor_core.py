...
from heron.proto import packing_plan_pb2
...
class HeronExecutor:
  def __init__(self, args):
    ...
    self.processes_to_monitor = {} # TODO: we need to synchronize rw to this dict since is used by multiple threads
  ...
  def parse_instance_distribution(self, encoded_distribution):
    ...
    f1 = map(lambda z: (int(z[0]), ':'.join(z[1:])), map(lambda x: x.split(':'), encoded_distribution.split(',')))
  ...
  def kill_processes(self, commands):
    ...
    for command_name, command in commands.iteritems():
      for p, name, exising_command, _ in self.processes_to_monitor.values():
  ...
  def register_packing_plan_watcher(self, executor):
      ...
      def on_packing_plan_watch(state_manager, packing_plan):
        do_print("State watch triggered for packing plan change. New PackingPlan: %s" % str(packing_plan))
        ...
        if current_distribution != new_distribution:
            do_print("Instance distribution change detected on shard %s, relaunching. Existing: %s, new: %s" %
                     (self.shard, current_distribution, new_distribution))
      ...
      for state_manager in state_managers:
        ...
        do_print("Registered state watch for packing plan changes with state manager %s." % str(state_manager))
