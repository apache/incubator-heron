...
# build heron
bazel --bazelrc=tools/travis-ci/bazel.rc build {heron,contrib}/...

# run heron unit tests
bazel --bazelrc=tools/travis-ci/bazel.rc test --test_tag_filters=-flaky {heron,contrib}/...

# flaky tests are often due to test port race conditions, which should be fixed. For now, run them serially
bazel --bazelrc=tools/travis-ci/bazel.rc test --test_tag_filters=flaky --jobs=0 {heron,contrib}/...
